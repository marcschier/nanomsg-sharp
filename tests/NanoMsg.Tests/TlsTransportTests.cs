...
public sealed class TlsTransportTests
{
    ...
    [Test]
    public async Task TlsTcp_push_pull_roundtrips()
    {
        ...
        int port = await pull.BindAsync("tls+tcp://127.0.0.1:0");
        push.Connect($"tls+tcp://127.0.0.1:{port}");
        await push.WaitForConnectionsAsync(1, Timeout);

        using CancellationTokenSource cts = new(Timeout);
        await push.SendAsync("over-tls"u8.ToArray(), cts.Token);
        using NanoMessage message = await pull.ReceiveAsync(cts.Token);
        await Assert.That(Encoding.ASCII.GetString(message.Span)).IsEqualTo("over-tls");
    }

    [Test]
    public async Task TlsTcp_pair_exchanges_both_ways()
    {
        using X509Certificate2 cert = TestTls.CreateSelfSignedCertificate();
        NanoSocketOptions serverOptions = new() { TlsServerCertificate = cert };
        NanoSocketOptions clientOptions = new()
        ...
        int port = await server.BindAsync("tls+tcp://127.0.0.1:0");
        client.Connect($"tls+tcp://127.0.0.1:{port}");
    ...
    [Test]
    public async Task Binding_tls_without_certificate_throws()
    {
        await using PullSocket pull = new();
        bool threw = false;
        try
        {
            await pull.BindAsync("tls+tcp://127.0.0.1:0");
        }
